...
export default function MobileSearch ({ children }: {children: React.ReactNode}) {
  const [value, setValue] = useState('')

  return (
    <>
      <div className='absolute top-[80px] left-0 bg-white md:hidden w-full'>
        ...
        <span className='w-full h-[45px] relative max-w-[475px] m-auto shrink-2 block z-10 mb-10'>
          <input value={value} onChange={(e) => setValue(e.target.value)} placeholder='Buscar' className='border-[1px] block pl-12 lg:text-base text-sm border-softWhite bg-transparent rounded-[50px] py-[10px] pr-[90px] w-full h-full sm:placeholder:text-sm lg:placeholder:text-base placeholder:text-white outline-none text-black placeholder:font-medium'/>
